...
function pushUpdates(
	...
	return new Promise(function(resolve) {
		socket.emit('pushUpdates', file, version, JSON.stringify(updates));
...
function pullUpdates(
	...
	return new Promise(function(resolve) {
		socket.emit('pullUpdates', file, version);
...
export function getDocument(socket: Socket, file: string): Promise<{version: number, doc: Text}> {
	return new Promise(function(resolve) {
		socket.emit('getDocument', file);
...
export const peerExtension = (socket: Socket, file: string, startVersion: number, id: string) => {
	let plugin = ViewPlugin.fromClass(class {
		...
		async push() {
			...
			await pushUpdates(socket, file, version, updates);
			this.pushing = false;
		...
		async pull() {
			while (!this.done) {
				let version = getSyncedVersion(this.view.state)
				let updates = await pullUpdates(socket, file, version)
